...
object DockerTools {
  ...
  val startKafkaContainer : IO[Unit] = acquireProcess("docker-compose", Seq("-f", dockerComposeFile, "up"), "Log loaded for partition test-topic-0 with initial high watermark")
    .map(_ => ())
